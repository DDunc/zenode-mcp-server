...
const TRACER_FIELD_DESCRIPTIONS = {
  prompt: "Detailed description of what to trace and WHY you need this analysis. MUST include context about what you're trying to understand, debug, analyze or find. For precision mode: describe the specific method/function and what aspect of its execution flow you need to understand. For dependencies mode: describe the class/module and what relationships you need to map. Example: 'I need to understand how BookingManager.finalizeInvoice method is called throughout the system and what side effects it has, as I'm debugging payment processing issues' rather than just 'BookingManager finalizeInvoice method'",
  trace_mode: "Trace mode: 'precision' (for methods/functions - shows execution flow and usage patterns) or 'dependencies' (for classes/modules/protocols - shows structural relationships)",
  images: "Optional images of system architecture diagrams, flow charts, or visual references to help understand the tracing context"
};

// Zod schema for request validation
const TracerRequestSchema = BaseToolRequestSchema.extend({
  prompt: z.string().describe(TRACER_FIELD_DESCRIPTIONS.prompt),
  trace_mode: z.enum(['precision', 'dependencies']).describe(TRACER_FIELD_DESCRIPTIONS.trace_mode),
  images: z.array(z.string()).default([]).describe(TRACER_FIELD_DESCRIPTIONS.images)
});

type TracerRequest = z.infer<typeof TracerRequestSchema>;

export class TracerTool extends BaseTool {
  name = 'tracer';
  ...
  defaultTemperature = 0.0; // Not used for this prompt generator
  modelCategory = ToolModelCategory.FAST;

  getZodSchema() {
    return TracerRequestSchema;
  }

  getSystemPrompt(): string {
    return ''; // Not used in this simplified tool
  }

  async execute(args: TracerRequest): Promise<ToolOutput> {
    const request = TracerRequestSchema.parse(args);

    logger.debug(`[TRACER] Generating ${request.trace_mode} mode analysis prompt`);

    // Create enhanced prompt with specific instructions
    const enhancedPrompt = this.createEnhancedPrompt(request.prompt, request.trace_mode);

    // Get rendering instructions
    const renderingInstructions = this.getRenderingInstructions(request.trace_mode);

    // Create response with both the enhanced prompt and instructions
    const responseContent = `THIS IS A STATIC CODE ANALYSIS REQUEST:
...
immediately then do not stop after displaying your output, proceed directly to your next step.`;
    ...
    return this.formatOutput(responseContent);
  }

  /**
   * Create an enhanced, specific prompt for analysis.
   */
  private createEnhancedPrompt(originalPrompt: string, traceMode: string): string {
    const modeGuidance = {
...
${modeGuidance[traceMode as keyof typeof modeGuidance]}
...
Provide concrete, code-based evidence for all findings. Reference specific line numbers and include exact method signatures. Identify uncertain paths where parameters or runtime context affects flow. Consider project scope and architectural patterns (monolith, microservices, layered, etc.).`;
   ...
   */
  private getRenderingInstructions(traceMode: string): string {
    if (traceMode === 'precision') {
      return this.getPrecisionRenderingInstructions();
    } else {
      return this.getDependenciesRenderingInstructions();
    }
  }
   ...
   */
  private getPrecisionRenderingInstructions(): string {
    return `
...
- Include comprehensive side effects categorization (database, network, filesystem, state, memory)`;
   ...
   */
  private getDependenciesRenderingInstructions(): string {
    return `
